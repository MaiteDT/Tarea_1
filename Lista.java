...
public class Lista<T> implements Listable<T>, Iterable<T>, Serializable {
    ...
    private class IteradorLista<T> implements Iterator<T> {
        ...
        @Override
        public T next() {
            final T elemen = siguiente.elemento;
     ...
     **/
    public Lista(final Lista<T> l) {
        final Lista<T> nueva = l.copia();
     ...
     **/
    public Lista(final Iterable<T> iterable) {
        longitud = 0;
        cabeza = null;
        cola = null;
     ...
     */
    public void agregarAlInicio(final T elemento) {
        final Nodo nuevo = new Nodo(elemento);
        if (esVacia()) {
            cabeza = cola = nuevo;
        } else {
     ...
     * @param elemento Objeto que se agregará al inicio de la lista.
     */
    public void agregarAlFinal(final T elemento) {
        final Nodo nuevo = new Nodo(elemento);
        if (esVacia()) {
            cabeza = cola = nuevo;
            longitud++;
     ...
     */
    public Nodo buscaNodo(final T elem) {
        Nodo aux = cabeza;
        while (aux != null) {
     ...
     *         caso.
     */
    public boolean contiene(final T elemento) {
     ...
     */
    public void eliminar(final T elemento) {
        final Nodo n = buscaNodo(elemento);
        if (n != null) {
            if (longitud == 1) {
                vaciar();
                return;
     ...
     *         ésta.
     */
    public int indiceDe(final T elemento) {
        if (contiene(elemento) != false) {
            final Nodo aux = new Nodo(elemento);
            int contador = 0;
     ...
     */
    public T getElemento(final int i) throws IndexOutOfBoundsException {
        if (i < 0 || i >= longitud) {
    ...
    public <T extends Comparable> Lista<T> merge(final Lista<T> l1, final Lista<T> l2) {
        final Lista<T> l = new Lista<>();
     ...
     */
    public static <T extends Comparable<T>> Lista<T> mergesort(final Lista<T> l) {
        if (l.longitud < 2) {
            return l;
        }
        int contador = 0;
        final Lista l1 = new Lista<>();
        final Lista l2 = new Lista<>();
        for (final T elem : l) {
